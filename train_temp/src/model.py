...
class RWKV_Tmix_x070(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            # KVW-Decay (Quail rand_vec_ideas_en.md, ★★)
            # Decay logit becomes state-aware: combines base w-logit with k and v.
            # Added in pre-exp(-exp) (logit) space so the existing CUDA kernel is reused unchanged.
            # Zero-init -> identical to baseline RWKV-7 at step 0; learns to modulate during training.
    ...
    def forward(self, x, v_first):
        ...
        ############################################################
        # KVW-Decay: state-aware decay using w, k, v (uses post-residual v)
        # Reference: rand_vec_ideas_en.md ★★ (Qwen3 32B, 2026-04-28)
        # The model now decides decay based on what is being written into memory,
        # not just the input shift. Modulation is added in logit space so the
        # CLAMPW kernel's exp(-exp(.)) still applies. b_ww=0 & g_ww=0 at init
        # so behavior matches baseline RWKV-7 at step 0.
        wkv_concat = torch.cat([w, k, v], dim=-1)  # [B, T, 3C]
        w = w + (wkv_concat @ self.g_ww + self.b_ww)  # [B, T, C]
        ############################################################
